...
@section('title', ($episode->title ?? ('Tập '.$episode->episode_number)).' | '.optional($episode->season->movie)->title)
...
.movie-content { position: relative; z-index: 10; }
.movie-poster {
  border-radius: 12px;
  box-shadow: 0 4px 15px rgba(0, 0, 0, 0.4);
}
.btn-play,.btn-action-group .btn-play{
  ...
  box-shadow:0 5px 15px rgba(253,219,0,.4);
  transition:all .3s ease;
}
.btn-play:hover,.btn-action-group .btn-play:hover{
  transform:translateY(-2px); box-shadow:0 8px 20px rgba(253,219,0,.6);
}
.btn-play .bi-play-fill,.btn-action-group .btn-play .bi-play-fill{ font-size:1.5rem; margin-right:.5rem; }

.btn-action-icon{ background:none;border:none;color:#d1d5db;font-size:14px;cursor:pointer;text-align:center; }
.btn-action-icon .bi{ font-size:1.5rem;display:block; }

/* === NAVIGATION & UTILITY === */
.nav-tabs{ border-bottom:2px solid #2c333a; }
.nav-tabs .nav-link{ color:#d1d5db;border:none;background:none; }
.nav-tabs .nav-link.active{
  background-color:#2c333a!important;border-color:#2c333a;color:#fff;border-radius:8px 8px 0 0;
}
.dropdown-item{ color:#d1d5db; }
.dropdown-item:hover{ background-color:#2c333a;color:#fff; }
.dropdown-toggle::after{ display:none; }
.badge-rounded-pill-dark{ background-color:#1e252e;border:1px solid #2c333a;color:#d1d5db; }
...
.episode-button{
  ...
  background:#1a212a;border:1px solid #2a3441;color:#e5e7eb;
  border-radius:10px;padding:.5rem .75rem;text-decoration:none;
  transition:background .2s ease, transform .1s ease;
}
.episode-button:hover{ background-color:#2c333a; }
.episode-button.active{ border-color:#fddb00; color:#fff; }
.episode-button.disabled{ opacity:.6; pointer-events:none; }
.episode-button i{ margin-right:.45rem; font-size:1rem; }
...
.meta-pill{
  ...
  padding:.15rem .45rem;border-radius:10px;font-size:.8rem;font-weight:600
}
...
.aired-pill{
  ...
  border:1px solid rgba(255,153,0,.25);color:#ffb15a;border-radius:18px;
  padding:.35rem .6rem;font-size:.9rem;font-weight:600
}
.aired-pill i{font-size:1rem}

.chip-outline{
  border:1px solid rgba(255,255,255,.7);color:#fff;background:transparent;border-radius:12px;
  padding:.35rem .65rem;display:inline-flex;align-items:center;gap:.4rem
}
.chip-outline strong{font-weight:700}
.chip-outline + .chip-outline{margin-left:.5rem}

/* === COMMENT SECTION === */
.comment-section{ margin:8px 0!important; }
.comment-section h4{ color:#fff;font-weight:600;display:flex;align-items:center;gap:10px; }
.comment-section .comment-tabs{ display:inline-flex;border:1px solid #3d4a57;border-radius:8px;overflow:hidden; }
.comment-section .comment-tabs button{
  background-color:#1e252e;border:none;color:#d1d5db;padding:8px 15px;font-size:1rem;transition:background-color .3s;
}
.comment-section .comment-tabs button.active{ background-color:#3d4a57;color:#fff; }

.comment-input-area{
  background-color:#1e252e;border-radius:8px;padding:1rem;border:1px solid #3d4a57;margin-top:10px!important;
}
.comment-input-area .inner-box{ background-color:#1a1a1a;border-radius:8px;padding:1rem; }
.comment-input-area textarea{
  background-color:transparent;border:none;color:#d1d5db;width:100%;resize:vertical;padding:0;outline:none;
}
.comment-input-area .comment-actions{ display:flex;justify-content:flex-end;align-items:center;margin-top:1rem; }
.comment-input-area .comment-actions .btn-send{
  background:none;border:none;color:#fddb00;font-size:1.2rem;font-weight:bold;display:flex;align-items:center;gap:.5rem;cursor:pointer;transition:color .3s ease;
}
.comment-input-area .comment-actions .btn-send:hover{ color:#f2c700; }

.comment-list .comment-item{
  gap:15px;padding:1rem 0;border-bottom:1px solid #2c333a;position:relative;overflow:visible;
}
.comment-list .comment-item:last-child{ border-bottom:none; }
.comment-list .comment-avatar{ width:45px;height:45px;border-radius:50%;object-fit:cover; }
.comment-list .comment-content{ flex-grow:1;position:relative;overflow:visible; }
.comment-list .comment-header{ display:flex;align-items:center;gap:10px;margin-bottom:5px; }
.comment-list .comment-header .comment-author{ font-weight:bold;color:#fff; }
.comment-list .comment-header .comment-meta{ font-size:.8rem;color:#7f8994; }
.comment-list .comment-body p{ margin-bottom:5px; }
.comment-badge{ background-color:#2c333a;color:#d1d5db;padding:3px 8px;border-radius:12px;font-size:.8rem; }

.comment-actions{ display:flex;align-items:center;gap:16px;font-size:.9rem; }
.comment-actions .btn-like,.comment-actions .btn-dislike{
  color:#7f8994;transition:color .15s ease,font-weight .15s ease;font-weight:500;cursor:pointer;
}
.comment-actions .btn-like.active,.comment-actions .btn-dislike.active{
  color:var(--bs-warning,#fddb00);font-weight:700;
}
.comment-actions .btn-like i,.comment-actions .btn-dislike i{ color:inherit; }
.comment-actions button{ background:none;border:none;color:#7f8994;cursor:pointer;transition:color .3s; }
.comment-actions button:hover{ color:#d1d5db; }

.comment-reply{ margin-left:0!important; }
.replies-toggle{ font-size:.9rem;color:#fddb00;cursor:pointer;margin-top:5px;text-decoration:none;display:inline-flex;align-items:center;gap:.25rem; }
.replies-toggle i{ display:inline-block;transition:transform .3s; }
.replies-toggle.collapsed i{ transform:rotate(0); }
.replies-toggle:not(.collapsed) i{ transform:rotate(180deg); }
...
.g-tile{ position:relative; overflow:hidden; border-radius:12px; background:#12171d; border:1px solid rgba(255,255,255,.08); }
.g-tile.poster{ aspect-ratio:2/3; }
.g-tile.banner{ aspect-ratio:21/9; }
.gallery-grid .g-tile > img{
  position:absolute; inset:0; width:100%!important; height:100%!important; object-fit:cover; display:block; border-radius:0;
}
.gallery-grid img{ width:auto; height:auto; }

/* === MISC === */
.mention{ background:#2b3240; color:#cbd5e1; padding:2px 6px; border-radius:6px; font-size:.85rem; font-weight:500; }
.custom-toast{
  background:#222;color:#fff;border-radius:12px;padding:12px 16px;box-shadow:0 8px 24px rgba(0,0,0,.35);
  font-size:14px;margin-top:8px;display:flex;align-items:center;gap:8px;opacity:0;transform:translateX(100%);
  transition:all .35s ease;border:1px solid rgba(255,255,255,.08);
}
.custom-toast.show{ opacity:1; transform:translateX(0); }
.custom-toast .toast-icon{ font-size:1.1rem; }
...
@php
  // 🔥 Sử dụng route stream thay vì đường dẫn Storage trực tiếp
  // Route này sẽ gọi đến MovieController@streamEpisode để phát video
  $src = route('episodes.stream', $episode);
  
  // Kiểm tra xem episode có data video không (để hiển thị thông báo)
  $hasVideoData = !empty($episode->video_path) || !empty($episode->file_name);
@endphp

@if ($hasVideoData)
  {{-- Dùng $src là route stream, không cần thẻ <source> --}}
  <video controls class="w-100 rounded shadow" style="max-height:70vh" preload="metadata">
    <source src="{{ $src }}" type="video/mp4">
    Trình duyệt của bạn không hỗ trợ thẻ video.
  </video>
@else
  <div class="alert alert-secondary">Chưa có video cho tập này.</div>
@endif
...
<div class="col-md-3">
  <div class="text-center">
    ...
    <div class="meta-row mb-3 justify-content-center">
      ...
      @if($movie->is_series)
        <span class="meta-pill">{{ $seasonBadge ?? 'Phim bộ' }}</span>
        <span class="meta-pill">Tập: {{ $episodesCount ?? 0 }}{{ !empty($plannedEpisodes) ? ' / '.$plannedEpisodes : '' }}</span>
      @endif

      <span class="meta-pill">{{ $verLabel[$movie->version] ?? strtoupper($movie->version) }}</span>
    </div>
    ...
    @if($movie->is_series && $total > 0)
      <div class="aired-pill mb-3">
        <i class="bi bi-arrow-clockwise"></i> Đã chiếu: {{ $shown }} / {{ $total }} tập
      </div>
    @endif
  </div>
  ...
  <div class="mt-4 text-start">
    ...
    @if(!empty($movie->duration))
      <p class="mb-1"><strong>Thời lượng:</strong> {{ $movie->duration }}</p>
    @endif
    @if($movie->countries && $movie->countries->count())
      <p class="mb-1"><strong>Quốc gia:</strong> {{ $movie->countries->pluck('name')->join(', ') }}</p>
    @endif
    @if(!empty($movie->production))
      <p class="mb-0"><strong>Sản xuất:</strong> {{ $movie->production }}</p>
    @endif
  </div>
</div>
...
{{-- RIGHT --}}
<div class="col-md-9">
<div class="d-flex align-items-center gap-3 flex-md-nowrap flex-wrap mb-4">

  {{-- Yêu thích (đứng sau Thêm vào) --}}
  ...
  {{-- Thêm vào (đứng trước Yêu thích) --}}
  <div class="dropdown d-inline-block" id="addToListDropdownWrap">
    <button
    ...
    @php
  $userLists = \App\Models\Watchlist::where('user_id', auth()->id())
      ->withCount('movies') // <-- thêm dòng này
      ->latest()->get();
...
@endphp
    ...
    <div id="wlList" style="max-height: 260px; overflow:auto;">
      @forelse($userLists as $wl)
        <a href="#"
          ...
          <span class="wl-badge">{{ $wl->movies_count }} phim</span>
        </a>
      @empty
  ...
  <div class="text-end" style="min-width:180px;">
    <button id="openRatingModal" type="button" class="btn btn-sm btn-outline-light rating-btn-hover-effect"
            data-bs-toggle="modal" data-bs-target="#ratingModal">
      @for ($i = 1; $i <= 5; $i++)
        <i class="bi {{ $userStars >= $i ? 'bi-star-fill' : 'bi-star' }} text-warning rating-star"></i>
      @endfor
    </button>
    ...
    <div class="tab-pane fade show active" id="episodes" role="tabpanel" aria-labelledby="episodes-tab">
      <div class="season-bar">
        <div class="season-title"><i class="bi bi-list"></i> Phần {{ $currentSeason }} 
             @if($totalEpisodes > 0)
                <span class="ms-1 badge bg-secondary">{{ $totalEpisodes }} Tập</span>
             @endif
        </div>
...
(function(){
  ...
  const startAt   = {{ (int)($initialTime ?? 0) }};

  const canSave   = {{ auth()->check() ? 'true' : 'false' }};

  const saveUrl   = @json(route('watch.progress', $movie->slug));
  ...
  player.addEventListener('loadedmetadata', () => {
    ...
    try{
      ...
      if (startAt > 0 && startAt < (player.duration || 1e9)) player.currentTime = Math.max(0, startAt - 2);
  ...
  function updateTime(){
    ...
    const c=Math.floor(player.currentTime||0), t=Math.floor(player.duration||0);
  ...
  let ticking=false,lastSent=0;

  const sendFetch=async(force=false)=>{

    if(!canSave || !player.duration || player.duration===Infinity) return;

    const pos=Math.floor(player.currentTime||0), dur=Math.floor(player.duration||0);

    if(!force && Math.abs(pos-lastSent)<5) return; lastSent=pos;

    try{ await fetch(saveUrl,{method:'POST',credentials:'same-origin',

      headers:{'Content-Type':'application/json','Accept':'application/json','X-Requested-With':'XMLHttpRequest','X-CSRF-TOKEN':csrfToken},

      body: JSON.stringify({position:pos,duration:dur})}); }catch(_){}

  };

  const schedule=()=>{ if(!ticking){ ticking=true; setTimeout(()=>{ticking=false; sendFetch();},5000);} };

  player.addEventListener('timeupdate', schedule);

  player.addEventListener('pause',()=>sendFetch(true));

  player.addEventListener('ended',()=>sendFetch(true));



  // Leave page: beacon + cut stream

  function beacon(){

    if(!canSave || !player.duration) return;

    const pos=Math.floor(player.currentTime||0), dur=Math.floor(player.duration||0);

    const blob=new Blob([JSON.stringify({position:pos,duration:dur})],{type:'application/json'});

    navigator.sendBeacon(saveUrl, blob);

  }

  function cut(){ try{ player.pause(); player.removeAttribute('src'); player.load(); }catch(_){ } }

  window.addEventListener('pagehide', ()=>{ beacon(); cut(); });
  ...
  document.getElementById('shareBtn')?.addEventListener('click', async ()=>{
    ...
    try{ await navigator.share({ title:@json($movie->title), url:location.href }); }

    catch(_){ await navigator.clipboard?.writeText(location.href); alert('Đã copy liên kết!'); }

  });



  // Quick add to list (click item)

  document.getElementById('wlList')?.addEventListener('click', (e)=>{

    const a=e.target.closest('.wl-item'); if(!a) return; e.preventDefault();

    const id=a.dataset.id; if(!id) return;

    fetch(@json(route('watchlists.items.store',['watchlist'=>'__WL__'])).replace('__WL__',id),{

      method:'POST', headers:{'X-CSRF-TOKEN':csrfToken,'Accept':'application/json','Content-Type':'application/json'},

      body: JSON.stringify({ movie_id: {{ $movie->id }} })

    }).then(()=>{ a.classList.add('bg-success'); setTimeout(()=>a.classList.remove('bg-success'),800); });
