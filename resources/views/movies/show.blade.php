    ...
    <div class="container">
        ...
        @php
            ...
            if ((int)$movie->is_series === 1) {
                // Logic lấy tập phim
                $epNum   = (int) request('ep', 0);
                $episode = null;

                if ($epNum > 0) {
                    $episode = \App\Models\Episode::whereHas('season', fn($q) => $q->where('movie_id', $movie->id))
                        ->where('episode_number', $epNum)
                        ->first();
                }

                if (!$episode) {
                    $episode = \App\Models\Episode::select('episodes.*')
                        ->join('seasons','seasons.id','=','episodes.season_id')
                        ->where('seasons.movie_id', $movie->id)
                        ->where(function($q){
                            $q->whereNotNull('episodes.video_path')
                              ->orWhereNotNull('episodes.file_name');
                        })
                ...
                if ($episode) {
                    $videoSrc = route('episodes.stream', $episode);
                }
            } else {
                // Phim lẻ
                $videoSrc = route('movies.stream', $movie->slug);
            }
        @endphp

        <div class="mb-3">
            <video id="moviePlayer" controls preload="metadata"
            ...
            {{-- Bỏ qua timeDisplay và thông báo tiếp tục từ... để khớp mẫu --}}
        </div>
